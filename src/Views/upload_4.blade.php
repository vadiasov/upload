                    ...
                    <div class="box box-info">
                        <div class="box-header with-border" style="margin-bottom: 10px">
                            <h3 class="box-title">{{ $config['header'] . ': '}} <span
                                        style="color: #00c0ef">{{ $subTitle }}</span>{{ $config['header_second_part'] . ': '}}
                                <span style="color: #00c0ef">{{ $title }}</span></h3>
                        </div>
                        <!-- /.box-header -->
                        <!-- form start -->
                        <form class="form-horizontal dropzone" method="post" enctype="multipart/form-data"
                            ...
                            <input type="hidden" name="subParentId" value="{!! $subParentId !!}">
                        </form>
                        <!-- /.box-body -->
                        <div class="box-footer">
                            <a href="{{ action($config['backUrl'], [$parentId, $subParentId]) }}" class="btn btn-default">Back</a>
                        </div>
                        <!-- /.box-footer -->
                    </div>
                </div>
